...
<%	var JS_Condition = "(/(bmp|gif|jpg|png|tif)/gi).test(ext)"
	var File_List = " bmp, jpg, gif, png, tif " %>
	...
	function check(){	var P = document.getElementById('myFile');   var  m= P.value.toLowerCase();
		if (m.length<5){    alert(' Bạn chưa chọn file\n Click vào nút "Browse" để chọn file '); P.text='';  return false;	}
			...
			if (  <%=JS_Condition%>  ){	
				...
				return true; }
			else {	alert('Không được phép chọn các file có đuôi :  asp , php , cgi , aspx , asa , js , cmd, bat, dll , exe, msi...');
					return false;	}
		}	}
</script>

<form action="doup.asp?Name=<%=VBRequest("Name")%>" METHOD="POST" ENCTYPE="multipart/form-data" onSubmit="return check()">
	<table align=center>
