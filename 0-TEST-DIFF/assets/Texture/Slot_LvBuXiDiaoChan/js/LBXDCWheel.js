...
cc.Class({
    ...
    onLoad() {
        this.audio = cc.director.getScene().getChildByName('Canvas').getComponent('LBXDCAudio');
        this.mainObj = cc.director.getScene().getChildByName('Canvas').getComponent('LBXDCMain');
    ...
    startRoll(...args) {
        ...
        setTimeout(() => {
            ...
            this.node.runAction(
                cc.sequence(
                    cc.delayTime(0.2),
                    cc.moveTo(timer, cc.v2(this.node.x, -this.node.height + 576)),
                    cc.moveTo(0.1, cc.v2(this.node.x, -this.node.height + 610)),
                    cc.moveTo(0.1, cc.v2(this.node.x, -this.node.height + 576)),
    ...
    stopImmediately() {
        ...
        setTimeout(() => {
            this.node.y = -this.node.height + 576;
        }, 50);
        this.status = 0;
        this.mainObj.stateCallBack();
    },
