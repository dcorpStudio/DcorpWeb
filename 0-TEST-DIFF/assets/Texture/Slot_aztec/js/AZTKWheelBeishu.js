...
cc.Class({
    ...
    startRoll(args) {
        ...
        this.addRole(this.getRandomId());
        let indexArray = [0, 1, 2, 3, 5, 10, 15];
        for (let i = 0; i < indexArray.length; i++) {
            if (indexArray[i] == args) {
                this.addRole(i);
                break;
            }
        }
        ...
        setTimeout(() => {
            ...
            this.node.runAction(
                cc.sequence(
                    cc.delayTime(0.2),
                    cc.moveTo(timer, cc.v2(this.node.x, -this.node.height + 595)),
                    cc.moveTo(0.1, cc.v2(this.node.x, -this.node.height + 650)),
                    cc.moveTo(0.1, cc.v2(this.node.x, -this.node.height + 595)),
    ...
    addRole(id) {
        let pb = cc.instantiate(this.mainObj.roleBeishuPb[id]);
    ...
    stopImmediately() {
        ...
        setTimeout(() => {
            this.node.y = -this.node.height + 595;
        }, 50);
        this.status = 0;
        this.mainObj.stateCallBack();
    },
