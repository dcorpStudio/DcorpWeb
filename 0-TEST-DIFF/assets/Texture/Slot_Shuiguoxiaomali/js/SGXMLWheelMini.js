...
cc.Class({
    ...
    startRoll(args) {
        ...
        setTimeout(() => {
            ...
            this.node.runAction(
                cc.sequence(
                    cc.delayTime(0.2),
                    cc.moveTo(timer, cc.v2(this.node.x, -this.node.height + 415)),
                    cc.moveTo(0.1, cc.v2(this.node.x, -this.node.height + 430)),
                    cc.moveTo(0.1, cc.v2(this.node.x, -this.node.height + 415)),
    ...
    addRole(id) {
        let pb = cc.instantiate(this.mainObj.roleMiniPb[id]);
    ...
    stopImmediately() {
        ...
        setTimeout(() => {
            this.node.y = -this.node.height + 485;
        }, 50);
        this.status = 0;
    },
});