...
cc.Class({
            ...
            if (this.lotteryRes.viewarray.getFreeTime.bFlag) {
                if (this.freeTimes == 0) {
                    ...
                    this.scheduleOnce(() => {
                        ...
                        this.node.getChildByName("com_tianjiangcaishen").active = true;
                        this.closeShine();
                        this.startFreeGame();
                        this.chooseWildStart();
                    }, 5);
                } else {
                    this.freeTimes = this.lotteryRes.viewarray.getFreeTime.nFreeTime;
                    this.stopFree = false;
    ...
    chooseWildStart() {
        ...
        this.scheduleOnce(() => {
            this.node.getChildByName("com_tianjiangcaishen").getChildByName("caishenfangwei").active = false;
        }, 2.5);
    ...
    _onTouchEnd(event) {
        ...
        pos.x -= cc.winSize.width / 2;
        pos.y -= cc.winSize.height / 2;
        let children = this.node.getChildByName("com_tianjiangcaishen").getChildByName("gezhi").children;
        let name = 1;
        ...
        let beishuArray = [[], [1, 1, 2, 2, 3], [1, 2, 2, 2, 3], [2, 2, 3, 3, 5], [2, 3, 3, 5, 10]];
        this.chooseIndex = parseInt(name);
        let beishuShow = [1, 1, 2, 2, 3];
        ...
        for (let i = 0; i < 5; i++) {
            this.node.getChildByName("com_tianjiangcaishen").getChildByName("label" + i).getComponent(cc.Label).string = "x" + beishuShow[i];
        }
    },
