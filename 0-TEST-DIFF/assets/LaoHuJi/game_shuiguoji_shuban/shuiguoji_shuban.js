var zhuanpan = require("zhuanpan_shuban");
...
cc.Class({
    ...
    properties: {
        ...
        guessNode: cc.Node,
        guessNum_lab: cc.Label,//猜大小数字
        winCoin_lab: cc.Label,//转盘收益
    },

    start: function () {
        Global = require("PlayerInfo").getInstant;
        this.url = Lhjconfig.Server_IP + Lhjconfig.shuiguoji_shuban_Port;
        ...
        this.node.on('Event_StateChange', function (event) {
            ...
            switch (event.detail) {
                ...
                case 3:
                    ...
                    if (self._winCoin > 0) {
                       // self.guessNode.active = true;
                        //self.guessBet = 0;
                        //self.winCoin_lab.string = "当前注码:" + (self.guessBet + self._winCoin);
                        //动态加载中奖金币预制件
                        ...
                        //     var x = vector.x;
                        //     vector.x = vector.y; 
                        //     vector.y = -x;//补丁：旋转一下vector，因为这款游戏的Canvas旋转了
    ...
    onBtnClick_KaiShi: function () {
        var duzhu = this.GetDuZhu() * 0.01;
        if (duzhu <= 0) {
            this.PanelQingXiaZhu.active = true;//请下注界面
            return;
        }
        if (duzhu > Global.playerCoin) {
    ...
    chooseBS(event, type) {
        ...
        this.isPlay = true;
        let t = parseInt(type);//0-大  1-小
        shuiguoji_socket.reqBet('guess', JSON.stringify({ choosetype: t, bet: (this.guessBet + this._winCoin) * 100 }));
    },
    //押大小加注
    addGuess() {
        if (this.isPlay) {
        ...
        this.guessBet += 1;
        this.winCoin_lab.string = "当前注码:" + (this.guessBet + this._winCoin);
    },
    //押大小返回
    onGuess(dict, userdata) {
        this._winCoin = dict["win"];//保存服务器返回的本局赢多少钱
        Global.playerCoin = userdata.userscore;//更新游戏金币数量
        this.speed = 0.1;
        this.guessNum = 0;
        this.guessNum_lab.string = this.guessNum + 1;
        this.tw = () => {
            this.guessNum += 1;
            this.guessNum_lab.string = this.guessNum % 6 + 1;
            if (this.guessNum >= 30) {
                this.speed += 0.1;
            }
            if (this.guessNum >= 36 && dict["nWinLines"] == this.guessNum % 6 + 1) {
                cc.log(dict["nWinLines"]);
                this.LabelJinBi.string = Global.playerCoin;//总金币数
                this.LabelJiangLi.string = this._winCoin;//显示本局赢多少钱
                this.guessBet = 0;
                this.winCoin_lab.string = "当前注码:" + (this.guessBet + this._winCoin);
                this.isPlay = false;
            } else {
                setTimeout(this.tw, this.speed * 1000);
            }
        };
        setTimeout(this.tw, this.speed * 1000);
    },
